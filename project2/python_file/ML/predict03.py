...
# 1. 데이터 로드
tqdm.pandas()
df = pd.read_csv("csv/reviews_repredicted.csv")
print(df.info())

# 텍스트와 라벨
X = df['tokens'].astype(str)
y = df['pred_emotion']   # 0 / 1 라벨
...
print(pred_sentiment("맛없다 친절 하다"))
print(pred_sentiment("한참 먹다 삼계탕 바퀴벌레 나오다 먹다 식겁하다 황당하다 어렵다 자리 들다 크게 항의 못 먹다 사람 숟가락 놓다 밥값 받다"))
print(pred_sentiment("무한리필 계속 먹다 수 두번째 판 비주 얼다 달라지다 연어 껍질 부분 붙다 살 주다 더 비리 연어 살이 적다 느낌 원래 비주 얼 계속 괜찮다 금은 두번째 판 아쉽다 중간 열라면 먹다 딱이다 물이 한강 아쉽다"))
print(pred_sentiment("사진 두번째 리필 진키 오스 크다 리필 주문 하다 리필 해도 마음 불편하다 않다 주문 하다 퀄리티 좋다 연어 살 부위 경우 다르다 이해 하다 먹다 괜찮다 부위 나오다 좋다 차 끄다 오다 맥주 말다"))
"""
